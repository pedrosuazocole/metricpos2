...
function openExcelModal() { openModal('excel-modal'); }

document.addEventListener('DOMContentLoaded', () => {
  const input = document.getElementById('excel-file-input');
  if(input) input.addEventListener('change', e => leerArchivoExcel(e.target.files[0]));
  ...
  if(dz) {
    ...
    dz.addEventListener('drop', e => { e.preventDefault(); dz.style.borderColor='#e2e8f0'; leerArchivoExcel(e.dataTransfer.files[0]); });
  }
});
  ...
  reader.onload = function(e) {
    try {
      ...
      if(file.name.endsWith('.csv')) {
        ...
        procesarFilasExcel(rows);
      } else {
        // For xlsx we use a simple approach - read as text hoping it's readable
        const text = new TextDecoder('utf-8', {fatal:false}).decode(data);
        alert('Para archivos .xlsx, por favor guárdalos como CSV primero.\nFormato: codigo,nombre,categoria,precio_venta,costo,gravado');
      }
...
function procesarFilasExcel(rows) {
  ...
  const colMap = {};
  ['codigo','nombre','categoria','precio_venta','costo','gravado'].forEach(col => {
    const idx = header.indexOf(col);
    if(idx>=0) colMap[col] = idx;
  });
  if(colMap.codigo===undefined||colMap.nombre===undefined||colMap.precio_venta===undefined) {
    alert('Columnas requeridas: codigo, nombre, precio_venta\nEncabezados encontrados: '+header.join(', ')); return;
  }
  const productos = rows.slice(1).filter(r=>r[colMap.codigo]&&r[colMap.nombre]).map(r=>({
    codigo: r[colMap.codigo], nombre: r[colMap.nombre],
    categoria: colMap.categoria!==undefined ? r[colMap.categoria]||'General' : 'General',
    precio_venta: parseFloat(r[colMap.precio_venta])||0,
    costo: colMap.costo!==undefined ? parseFloat(r[colMap.costo])||0 : 0,
    gravado: colMap.gravado!==undefined ? (r[colMap.gravado]==='0'?0:1) : 1,
  }));
  excelDataCache = productos;
  const prev = document.getElementById('excel-preview');
  prev.style.display='block';
  prev.innerHTML = `<div style="background:#f8fafc;border-radius:8px;padding:12px;margin-bottom:10px">
    <div style="font-weight:700;color:#1e293b;margin-bottom:8px">Vista previa (${productos.length} productos)</div>
    <div style="max-height:180px;overflow-y:auto">
      <table style="width:100%;font-size:11px;border-collapse:collapse">
        <thead><tr style="background:#1e3a5f;color:#fff"><th style="padding:4px 6px">Código</th><th style="padding:4px 6px">Nombre</th><th style="padding:4px 6px">Categoría</th><th style="padding:4px 6px;text-align:right">Precio</th><th style="padding:4px 6px;text-align:right">Costo</th></tr></thead>
        <tbody>${productos.slice(0,10).map(p=>`<tr><td style="padding:3px 6px;font-family:monospace">${p.codigo}</td><td style="padding:3px 6px">${p.nombre}</td><td style="padding:3px 6px">${p.categoria}</td><td style="padding:3px 6px;text-align:right">${p.precio_venta.toFixed(2)}</td><td style="padding:3px 6px;text-align:right">${p.costo.toFixed(2)}</td></tr>`).join('')}${productos.length>10?`<tr><td colspan="5" style="text-align:center;padding:4px;color:#94a3b8">...y ${productos.length-10} más</td></tr>`:''}</tbody>
      </table>
    </div>
  </div>`;
...
async function importarProductosExcel() {
  if(!excelDataCache||!excelDataCache.length) return;
  const btn = document.getElementById('excel-import-btn');
  btn.disabled = true; btn.textContent = 'Importando...';
  try {
    const r = await POST('/productos/importar_excel', {productos: excelDataCache});
    const res = document.getElementById('excel-result');
    res.style.display='block';
    res.innerHTML = `<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px">
      <div style="font-weight:700;color:#16a34a;margin-bottom:4px">✅ Importación completada</div>
      <div style="font-size:12px;color:#15803d">Creados: ${r.creados} | Actualizados: ${r.actualizados} | Total: ${r.total}</div>
      ${r.errores?.length?`<div style="font-size:11px;color:#dc2626;margin-top:4px">Errores: ${r.errores.length}</div>`:''}
    </div>`;
    excelDataCache = null;
...
function setupEvents() {
  ...
  // Excel
  _on('excel-modal-cancel', 'click', () => { closeModal('excel-modal'); excelDataCache=null; document.getElementById('excel-preview').style.display='none'; document.getElementById('excel-result').style.display='none'; document.getElementById('excel-import-btn').style.display='none'; });
  // Editor Reportes
